<?php
/**
 * Droppy email library
 * Based on CodeIgniter email library
 *
 * @author Proxibolt
 * @version 1.0
 */
...
class Email {
	...
	public function __construct(array $config = array())
	{
        ...
        if($this->CI->config->item('email_server') == 'SMTP') {
            $config['protocol'] = 'smtp';

            $config['smtp_host'] = $this->CI->config->item('smtp_host');
            $config['smtp_port'] = $this->CI->config->item('smtp_port');
            $config['smtp_user'] = $this->CI->config->item('smtp_username');
            $config['smtp_pass'] = $this->CI->config->item('smtp_password');
            $config['smtp_crypto'] = ($this->CI->config->item('smtp_secure') == 'none' ? '' : $this->CI->config->item('smtp_secure'));
        }
        else
        {
	...
	/************************ Custom functions for Droppy ****************************/

    /**
     * Will fetch the email from the database and replaces all the placeholders
     *
     * @param $template
     * @param $data
     ...
     */
    public function createEmail($template, $data, $language = '') {
        ...
        $body_text      = $this->CI->templates->getByTypeAndLanguage($template, $language)['msg'];
        $subject_text   = $this->CI->templates->getByTypeAndLanguage($template . '_subject', $language)['msg'];

        // Remove password section if password isn't set
        if(!isset($data['password']) || empty($data['password'])) {
            $body_text = preg_replace("/{start_password}\r\n[\s\S]+?{end_password}\r\n/", '', $body_text);
            $body_text = preg_replace('/{start_password}[\s\S]+?{end_password}/', '', $body_text);
        }
        else
        {
            $body_text = str_replace(array("{start_password}\r\n", "{start_password}"), '', $body_text);
            $body_text = str_replace(array("{end_password}\r\n", "{end_password}"), '', $body_text);
        }

        // Replace placeholders in text
        $email['logo']      = $this->CI->config->item('site_url') . $this->CI->config->item('logo_path');
        $email['body']      = nl2br($this->textReplace($body_text, $data));
        $email['subject']   = $this->textReplace($subject_text, $data);
        $email['settings']  = $this->CI->config->config;

	    // Load the template and insert all the variables
	    $template = $this->CI->load->view('themes/' . $theme . '/emails/email.php', $email, TRUE);

	    // Set the template as body
    ...
    /**
     * Replaces placeholders with text
     *
     * @param $text
     * @param $replace
     * @return string
     */
    private function textReplace($text, $replace) {
        $pattern = '{%s}';
    ...
    /**
     * Puts everything together and sends the email
     *
     * @param null $template
     * @param array $data
     * @param array $to
     ...
     * @return bool
     */
    public function sendEmail($template = null, $data = array(), $to = array(), $language = '') {
        $this->clear();

        // Convert to array to comma separated list
        $strTo = implode(',', $to);

        // Get placeholders
        $placeholders = $this->getPlaceholders($data['upload_id'], $template, $data);

        // Create the email
        $this->createEmail($template, $placeholders, $language);
    ...
    /**
     * Send an clean email message (No text from database etc.)
     *
     * @param $message
     * @param $subject
     * @param array $to
     * @return bool
     */
    public function sendEmailClean($message, $subject, $to = array()) {
        // Convert to array to comma separated list
        $strTo = implode(',', $to);
        ...
        // Load the template and insert all the variables
        $message = $this->CI->load->view('themes/' . $theme . '/emails/email.php', array('body' => $message, 'logo' => $this->CI->config->item('site_url') . $this->CI->config->item('logo_path'), 'settings' => $this->CI->config->config), TRUE);
        ...
        $this->CI->logging->log("Sending email '$subject' to ".json_encode($to));
    ...
    /**
     * Generates receiver list by upload id
     *
     * @param $upload_id
     * @return string
     */
    private function genReceiverList($upload_id) {
        $this->CI->load->model('receivers');

        // Create receiver list
        $receivers = $this->CI->receivers->getByUploadID($upload_id);

        $receiver_list = '<ul>';
        foreach($receivers as $receiver) {
            $receiver_list .= '<li>'.$receiver['email'].'</li>';
        }
        $receiver_list .= '</ul>';
    ...
    /**
     * Generates a file list by upload id
     *
     * @param $upload_id
     * @return string
     */
    private function genFileList($upload_id) {
        $this->CI->load->model('files');

        $files = $this->CI->files->getByUploadID($upload_id);

        $file_list = '<ul style="max-width: 300px">';
        foreach($files as $file) {
            $file_list .= '<li>'.$file['file'].'</li>';
        }
        $file_list .= '</ul>';
     ...
     * @return string
     */
    private function secondsToReadable($seconds) {
        ...
        if($seconds >= 604800 && $seconds < 2592000) {
            ...
            {
                $ret .= "$weeks ".lang('weeks');
            }
        }
        if($seconds >= 2592000) {
            $months = intval(intval($seconds) / (3600*24*30));
            if($seconds == 2592000) {
                $ret .= "$months ".lang('month');
            }
            else
            {
                $ret .= "$months ".lang('months');
            }
        }
    ...
    /**
     * Return placeholders by email type
     *
     * @param $upload_id
     * @param $template
     ...
     * @return array|bool
     */
    public function getPlaceholders($upload_id, $template, $extra_data = array()) {
            ...
            switch($template) {
                case 'receiver':
                    $data = array(
                        ...
                        'upload_id'     => $upload_id,
                        'message'       => nl2br($upload['message']),
                        'total_files'   => $upload['count'],
                        'expire_time'   => $this->secondsToReadable(($upload['time_expire'] - $upload['time'])),
                ...
                case 'downloaded':
                    $data = array(
                        ...
                        'downloader_ip' => $extra_data['download_ip']
                    );
                break;
            }

            // Add extra data to data array
            if(!empty($extra_data)) {
